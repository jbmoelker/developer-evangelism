...
create('src/components/favicons/icon.png', getConfig())
    .then(response => {
        const dir = 'dist/';
        saveFiles(response.files, dir);
        saveFiles(response.images, dir);
        const html = response.html.join('\n').replace(/(href=")/gm, '$1{{ root }}');
        //require('fs').writeFileSync('src/components/favicons/favicons.html', html);
...
function getConfig() {
    ...
    return {
        appName: changeCase.title(pkg.name),    // Your application's name. `string`
